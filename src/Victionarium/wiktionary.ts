...
async function getLatinLemmaHTMLByTitle(title: string): Promise<LemmaResult> {
  const response = await fetch(`${WIKTIONARY_PAGE_API}/${title}`);
...
async function getRandomLatinLemmaHTML(): Promise<LemmaResult> {
  const response = await fetch(`${WIKTIONARY_ACTION_API}?${new URLSearchParams({
    action: "query",
  ...
  if(!response.ok) { throw response; }
  const members = await response.json().then((r: any) => r.query.categorymembers);
